...
private:
    ...
    bool pertenece(Atleta a, vector<Atleta> participan) const;
    bool pertenecePais(Pais p, vector<Pais> paisesls ) const;
    int contarpais(Pais p, vector<Pais> paisesls ) const;
    int contarAtleta(Atleta a, vector<Atleta> als) const;
	int sequedadOlimpica(Pais p) const;
    Atleta  atletaMaxCapacidad (vector<Atleta> participantes, Categoria c) const;
    vector<Atleta> rankingSacoElemento(vector <Atleta> ranking, Atleta a) const;
    vector<int> rankingOrdenado(vector<Atleta> participantes, Categoria c) const;
    pair<Pais, vector <int>> primerPais (vector<pair<Pais, vector <int>>> medallero) const;
    vector<pair<Pais, vector <int>>> medalleroSacoElemento(vector<pair<Pais, vector <int>>> medallero, pair<Pais, vector <int>> p ) const;
    vector<pair<Pais, vector <int>>> medalleroOrdenado(vector<pair<Pais, vector <int>>> medallero ) const;
